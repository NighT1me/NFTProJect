  ...
  setup() {
    ...
    return () => (
      <div ref={header} class={style.header}>
        ...
        <div>
          <div class={style.sing} onClick={() => {localStorage.setItem('Type','en');toUrl('/index-en');setTimeout(function(){location.reload()},100)}}>
          English
          </div>
          <div class={style.sing} onClick={() => {toPath('https://coself123-m.rarefy.top/#/');}}>
            {localStorage.getItem('Type')=='en'?'Market':'集市'}
          </div>
          <div class={style.sing} onClick={() => toPath('https://coself123-m.rarefy.top/#/')}>
            {localStorage.getItem('Type')=='en'?'LogIn':'登录'}
          </div>
